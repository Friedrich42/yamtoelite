...
client = Client(conf.API_TOKEN_YANDEX_MUSIC_CLIENT)
track_db = TrackDB()

main_channel_id = conf.CHANNEL_ID_FOR_BOT


def get_songs_from_yamusic():
    # liked = client.users_likes_tracks()
    # tracks = []
    # for song in liked.tracks:
    #     tracks.append(Track(id_of_song=song.track.id,
    ...
    try:
        return list([Track(id_of_song=song.track.id,
                           ...
                           artists=" ft. ".join([artist.name for artist in song.track.artists]),
                           album_id=song.album_id,
                           local_path="", ) for song in client.users_likes_tracks().tracks])
